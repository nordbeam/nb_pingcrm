defmodule NbPingcrm.Accounts do
  @moduledoc """
  ...
  def register_user(attrs) do
    Repo.transaction(fn ->
      ...
      account_name = attrs[:email] || attrs["email"] || "New Account"

      {:ok, account} =
        %Account{}
        |> Account.changeset(%{name: account_name})
        |> Repo.insert()
      ...
      case %User{}
           ...
           |> Ecto.Changeset.put_change(:owner, true)
           |> Repo.insert() do
        {:ok, user} -> user
  ...
  See `NbPingcrm.Accounts.User.email_changeset/3` for a list of supported options.
  ...
  See `NbPingcrm.Accounts.User.password_changeset/3` for a list of supported options.
  ...
  """
  def list_users(account_id, params \\ %{}, opts \\ []) do
    ...
    |> Flop.validate_and_run(params, Keyword.merge([for: User, filter_op: filter_op], opts))
  end

  @doc """
  Gets a single user within an account.
  """
  def get_user_for_account!(account_id, id) do
    User
    |> User.for_account(account_id)
    |> Repo.get!(id)
  end
  ...
  @doc """
  Counts active (non-deleted) users in an account.
  """
  def count_users(account_id) do
    User
    |> User.for_account(account_id)
    |> User.filter_trashed(nil)
