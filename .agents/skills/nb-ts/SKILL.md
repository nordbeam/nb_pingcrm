...
- If validation reports a missing platform binary, expect one immediate actionable error and no retry/download loop. Check the detected path, provision only that platform (or configure a trusted managed executable), then retry. If the generated bundle still fails, compile/load every companion package before regenerating and inspect the first reported source contract.
